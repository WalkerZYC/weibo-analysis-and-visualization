...
result_df = result_df[
        (result_df['replytime'] >= 1000000000) & (result_df['replytime'] <= 2699893811)]
result_df['updatetime'] = pd.to_datetime(result_df['updatetime'], unit='s', errors='coerce').dt.strftime(
        '%Y%m%d')
result_df['replytime'] = pd.to_datetime(result_df['replytime'], unit='s', errors='coerce').dt.strftime(
        '%Y%m%d')
result_df['updatetime'] = pd.to_numeric(result_df['updatetime'], errors='coerce')
result_df['replytime'] = pd.to_numeric(result_df['replytime'], errors='coerce')
...
if __name__ == '__main__':
    ...
    between_20220901_and_20230901 =(nan_result_df)[(nan_result_df['replytime'] >= 20220901) & (nan_result_df['replytime'] <= 20230901)]
    nan_result_df=between_20220901_and_20230901
